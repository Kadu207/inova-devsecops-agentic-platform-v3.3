...
$composeBase = @("compose")
if (Test-Path (Join-Path $root ".env.staging")) {
  $composeBase += @("--env-file", ".env.staging", "--profile", "staging")
}

function Invoke-Compose {
  param([string[]]$CommandArgs)
  & docker @composeBase @CommandArgs
}
...
for ($i = 0; $i -lt 60; $i++) {
  Invoke-Compose @("exec", "-T", "postgres", "pg_isready", "-U", "inova", "-d", "inova_platform") 2>$null | Out-Null
  $nats = Invoke-Compose @("ps", "nats", "--format", "{{.Health}}") 2>$null
...
for ($i = 0; $i -lt 45; $i++) {
  ...
  foreach ($svc in $required) {
    $st = Invoke-Compose @("ps", $svc, "--format", "{{.Status}}") 2>$null
...
Invoke-Compose @(
  "exec", "-T", "postgres", "psql", "-U", "inova", "-d", "inova_platform", "-c",
  @"
...
$completedWorkers = Invoke-Compose @(
  "exec", "-T", "postgres", "psql", "-U", "inova", "-d", "inova_platform", "-t", "-A", "-c",
  @"
...
if ([int]$completedWorkers -lt $MinCompletedWorkers) {
  ...
  Invoke-Compose @("logs", "orchestrator", "sonar-worker", "snyk-worker", "datadog-worker", "--tail=50")
  exit 1
}
